...
const CheckoutFR = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">Ajoutez des produits avant de continuer</p>
          <Link to="/fr/catalogo" className="btn-primary">Voir le Catalogue</Link>
        </div>
      </div>
    );
  }
  ...
  const steps = [{ id: 'info', label: 'Information' }, { id: 'shipping', label: 'Livraison' }, { id: 'payment', label: 'Paiement' }];
      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="section-container">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="text-gray-600 mb-2">Votre numéro de commande est: <span className="font-semibold">#{Date.now().toString().slice(-8)}</span></p>
            <p className="text-gray-600 mb-8">Nous avons envoyé un email de confirmation à <span className="font-semibold">{formData.email}</span></p>
            <div className="bg-white rounded-xl p-6 shadow-sm mb-8 text-left">
              ...
              <ul className="space-y-3">
                <li className="flex items-start gap-3"><div className="w-6 h-6 bg-[#e30614] text-white rounded-full flex items-center justify-center text-sm flex-shrink-0">1</div><span className="text-gray-600">Vous recevrez un email avec les détails de votre commande</span></li>
                <li className="flex items-start gap-3"><div className="w-6 h-6 bg-[#e30614] text-white rounded-full flex items-center justify-center text-sm flex-shrink-0">2</div><span className="text-gray-600">Notre équipe vous contactera pour confirmer la personnalisation</span></li>
                <li className="flex items-start gap-3"><div className="w-6 h-6 bg-[#e30614] text-white rounded-full flex items-center justify-center text-sm flex-shrink-0">3</div><span className="text-gray-600">Nous vous enverrons un échantillon numérique avant production</span></li>
              </ul>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/fr/" className="btn-outline">Retour à l'accueil</Link>
              <Link to="/fr/catalogo" className="btn-primary">Continuer les achats</Link>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="section-container py-4">
          <div className="flex items-center justify-between">
            <Link to="/fr/carrito" className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors"><ArrowLeft className="w-5 h-5" /><span>Retour au panier</span></Link>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {step === 'shipping' && (
                <div className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
                  ...
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Notes de commande</label><textarea name="notes" value={formData.notes} onChange={handleInputChange} rows={3} className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-[#e30614] focus:ring-2 focus:ring-[#e30614]/20 outline-none transition-all resize-none" placeholder="Instructions spéciales de livraison, etc." /></div>
                  <div className="flex gap-4">
                    <button type="button" onClick={() => setStep('info')} className="flex-1 btn-outline py-4">Retour</button>
                    <button type="submit" className="flex-1 btn-primary py-4">Continuer au paiement</button>
                  </div>
                </div>
              )}
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#e30614] transition-colors">
                      ...
                      <div className="flex-1"><span className="font-medium text-gray-900">Virement bancaire</span><p className="text-sm text-gray-500">Les coordonnées bancaires seront envoyées par email</p></div>
                    </label>
                    <label className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#e30614] transition-colors">
                      <input type="radio" name="paymentMethod" value="card" className="w-5 h-5 text-[#e30614]" />
                      <div className="flex-1"><span className="font-medium text-gray-900">Carte de crédit/débit</span><p className="text-sm text-gray-500">Paiement sécurisé avec Stripe</p></div>
                      <CreditCard className="w-6 h-6 text-gray-400" />
                    </label>
                    <label className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#e30614] transition-colors">
                      <input type="radio" name="paymentMethod" value="paypal" className="w-5 h-5 text-[#e30614]" />
                      <div className="flex-1"><span className="font-medium text-gray-900">PayPal</span><p className="text-sm text-gray-500">Paiement rapide et sécurisé</p></div>
                    </label>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-4">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" required className="mt-1 w-5 h-5 text-[#e30614] rounded" />
                      <span className="text-sm text-gray-600">J'ai lu et j'accepte les <Link to="/fr/nota-legal" className="text-[#e30614] underline">conditions générales</Link> et la <Link to="/fr/nota-legal" className="text-[#e30614] underline">politique de confidentialité</Link>.</span>
                    </label>
                  </div>
                  <div className="flex gap-4">
                    <button type="button" onClick={() => setStep('shipping')} className="flex-1 btn-outline py-4">Retour</button>
                    <button type="submit" disabled={isProcessing} className="flex-1 btn-primary py-4 flex items-center justify-center gap-2">
                      {isProcessing ? <><span className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full" />Traitement...</> : <><span>Confirmer la commande</span><span className="font-bold">{formatPrice(total)}</span></>}
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl p-6 shadow-sm sticky top-32">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-gray-600"><span>Sous-total ({itemCount} produits)</span><span>{formatPrice(subtotal)}</span></div>
                <div className="flex justify-between text-gray-600"><span className="flex items-center gap-1"><Truck className="w-4 h-4" />Livraison</span><span className={shipping === 0 ? 'text-green-600 font-medium' : ''}>{shipping === 0 ? 'Gratuite' : formatPrice(shipping)}</span></div>
                <div className="flex justify-between text-gray-600"><span>TVA (21%)</span><span>{formatPrice(tax)}</span></div>
                <hr className="border-gray-100" />
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 space-y-3">
                <div className="flex items-center gap-3 text-sm text-gray-600"><Shield className="w-5 h-5 text-green-500" /><span>Paiement 100% sécurisé</span></div>
                <div className="flex items-center gap-3 text-sm text-gray-600"><Truck className="w-5 h-5 text-blue-500" /><span>Livraison en 24-48h</span></div>
              </div>
            </div>
          </div>
        </div>
