...
const ProductoPT = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="section-container py-16 text-center">
          ...
          <Link to="/pt/catalogo" className="btn-primary">
            Ver Catálogo
          </Link>
        </div>
      </div>
  ...
  if (product.is_new) badges.push({ text: 'Novo', color: 'bg-green-500' });
  if (product.is_eco) badges.push({ text: 'Eco-Friendly', color: 'bg-emerald-500' });
  if (product.is_eu_made) badges.push({ text: 'Fabricado UE', color: 'bg-blue-500' });
  if (hasDiscount) badges.push({ text: 'Promoção', color: 'bg-red-500' });
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="section-container py-4">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Link to="/pt/" className="hover:text-[#e30614] transition-colors">Início</Link>
            <span>/</span>
            <Link to="/pt/catalogo" className="hover:text-[#e30614] transition-colors">Catálogo</Link>
            <span>/</span>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2 text-sm text-gray-600"><Package className="w-4 h-4 text-[#e30614]" /><span>Em stock</span></div>
              <div className="flex items-center gap-2 text-sm text-gray-600"><Truck className="w-4 h-4 text-[#e30614]" /><span>Envio 24-48h</span></div>
              <div className="flex items-center gap-2 text-sm text-gray-600"><Shield className="w-4 h-4 text-[#e30614]" /><span>Garantia incluída</span></div>
              {product.is_customizable && <div className="flex items-center gap-2 text-sm text-gray-600"><Check className="w-4 h-4 text-[#e30614]" /><span>Personalizável</span></div>}
            </div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              {product.quantity_prices?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <p className="text-sm font-medium text-gray-700 mb-2">Preços por quantidade:</p>
                  <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm text-gray-500">Mín. {product.min_quantity} unidades</span>
            </div>
            ...
            <button onClick={handleAddToCart} disabled={isAdding} className={`w-full py-4 rounded-xl font-semibold text-lg flex items-center justify-center gap-3 transition-all ${added ? 'bg-green-500 text-white' : 'bg-[#e30614] text-white hover:bg-[#c7000b]'}`}>
              {added ? <><Check className="w-6 h-6" />Adicionado ao carrinho</> : <><ShoppingCart className="w-6 h-6" />{isAdding ? 'A adicionar...' : 'Adicionar ao Carrinho'}</>}
            </button>
